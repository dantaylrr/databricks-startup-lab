...
-- MAGIC """
-- MAGIC
-- MAGIC Use this block to make notes on question above:
-- MAGIC
-- MAGIC """
...
-- MAGIC
-- MAGIC **3.1).** Now, stand this data up (as a **one off batch operation**) in the same catalog and schema whilst filtering for the relevant conditions you've identified from exercise **1.3).**
...
-- MAGIC
-- MAGIC An additional requirement is that any table that we produce as part of this exercise **must be stripped of any customer PII** to ensure that we meet compliance standards.
-- MAGIC
...
-- MAGIC * [On the fly PII masking using DBSQL AI functions](https://docs.databricks.com/en/large-language-models/ai-functions.html) (Subject to region availability).
...
-- MAGIC %md-sandbox
-- MAGIC
-- MAGIC ## We want our gold table to have the following schema:
-- MAGIC
-- MAGIC ```bash
...
-- MAGIC ### Optional.
-- MAGIC
-- MAGIC **5.2).** Implement a new notebook & workflow that conforms to this contraint using either:
-- MAGIC
-- MAGIC * Spark structured-streaming in Python.
-- MAGIC * [Streaming tables](https://docs.databricks.com/en/sql/language-manual/sql-ref-syntax-ddl-create-streaming-table.html) (support for these is extremely limited in non-DLT & serverless SQL warehouse workloads as of writing).
-- MAGIC
